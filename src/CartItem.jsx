...
const CartItem = ({ onContinueShopping }) => {
  ...
  const handleIncrement = (item) => {
    dispatch(updateQuantity({ ...item, quantity: item.quantity + 1 })); // Increment quantity
  };

  const handleDecrement = (item) => {
    if (item.quantity > 1) {
      dispatch(updateQuantity({ ...item, quantity: item.quantity - 1 })); // Decrement quantity
    } else {
  ...
  const calculateTotalCost = (item) => {
    const cost = parseFloat(item.cost.substring(1)); // Convert "$10.00" to 10.00
    return (item.quantity * cost).toFixed(2); // Return total cost with 2 decimal places
  };
